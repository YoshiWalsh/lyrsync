...
function init() {
    ...
    const lyricsFile = document.querySelector("link.lyricsFile").href;
...
function initialiseYoutubePlayer() {
    return new Promise((resolve, reject) => {
        ytReady.then(() => {
            const youtubePlayer = document.querySelector(".youtubeContainer .player");
    ...
    for(let i = 0; i < lyricsFile.length; i++) {
        ...
        switch(char) {
            ...
            case "<":
                ...
                timecodeMatches = tagContents.match(timecodeRegex);
                if(timecodeMatches) {
...
function layoutLyrics() {
    ...
    const cardElms = Array.from(container.querySelectorAll(".card"));
    cardElms.forEach(cardElm => {
        const voiceElms = Array.from(cardElm.querySelectorAll(".voice"));
    ...
    for(let i = 0; i < cardElms.length; i++) {
        ...
        const separators = Array.from(cardElm.querySelectorAll(".voiceSeparator"));
...
function redraw(now) {
    ...
    for(let i = 0; i < activeCards.length; i++) {
        ...
        const separators = Array.from(contentsElm.children).filter((_, index) => (index % 2));
        ...
        for(let i = 0; i < voices.length; i++) {
            ...
            const words = Array.from(voiceElm.children);
